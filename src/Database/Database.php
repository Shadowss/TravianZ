...
final class Database implements IDbConnection
{
    ...
    function login($username, $password)
    {
        ...
        if ($pwOk) {
            ...
            if (!$dbarray['is_bcrypt'] && !$bcrypted) {
                mysqli_query($this->dblink, "UPDATE " . TB_PREFIX . "users SET password = '" . password_hash($password, PASSWORD_BCRYPT, [
                    'cost' => 12
                ]) . "'" . ($bcrypt_update_done ? ', is_bcrypt = 1' : '') . " where id = " . (int) $dbarray['id']);
            }
            $cachedResult = true;
        } else {
            $cachedResult = false;
    ...
    function getSharedForums($uid, $alliance)
    {
        ...
        return [
            'alliance' => $allianceForums,
            'confederation' => $confForums,
            'closed' => $closedForums
        ];
    }
    ...
    {        
        ...
        if ($access == ADMIN && ADMIN_RECEIVE_SUPPORT_MESSAGES && in_array($mode, [
            1,
            2,
            6,
            9,
        ...
        if ($access == MULTIHUNTER && in_array($mode, [
            1,
            2,
            6,
            9,
    ...
    function removeBuilding($d, $tribe, $wid, $fieldsArray = [])
    {
        ...
        if ($canBeRemoved && $jobToDelete['field'] > 18 && $jobToDelete['field'] != 99 && $jobToDelete['level'] - 1 == 0) {
            $this->setVillageLevel($wid, [
                "f" . $jobToDelete['field'] . "t"
            ], [
                0
            ]);
        }
    ...
    function getBuildingByType($wid, $type)
    {
        ...
        $type = (strpos($type, ',') === false ? [
            (int) $type
        ] : explode(',', str_replace(' ', '', $this->escape($type))));
    ...
    function resetPassword($uid, $cpw)
    {
        ...
        if (!empty($dbarray)) {
            if (!$this->updateUserField($uid, 'password', password_hash($dbarray['npw'], PASSWORD_BCRYPT, [
                'cost' => 12
            ]), 1))
                return false;
    ...
    function getPrisoners($wid, $mode = 0, $use_cache = true)
    {
        ...
        if (!$array_passed) {
            ...
            self::$prisonersCache[$wid[0] . $mode] = (count($result) ? [
                $result
            ] : []);
        } else {
            if ($result && count($result)) {
