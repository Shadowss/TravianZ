...
if (
    isset($_POST['medalid']) &&
    !empty($_POST['medalid']) &&
    is_numeric($_POST['medalid'])
) {
    $medalID = (int)$_POST['medalid'];

    mysqli_query(
        $database->dblink,
        "UPDATE " . TB_PREFIX . "medal SET del = 1 WHERE id = " . $medalID
    );
} elseif (
    isset($_POST['userid']) &&
    !empty($_POST['userid']) &&
    ...
    mysqli_query(
        $database->dblink,
        "UPDATE " . TB_PREFIX . "medal SET del = 1 WHERE userid = " . $userID
    );
}
...
$name = $database->escape(
    (string)$database->getUserField($uid, 'username', 0)
);
...
mysqli_query(
    $database->dblink,
    "INSERT INTO " . TB_PREFIX . "admin_log VALUES " .
    "(0, $admid, 'Deleted medal id [#$medalLogId] from the user " .
    "<a href=\\'admin.php?p=player&uid=$uid\\'>$name</a> ', " .
    time() . ")"
);
