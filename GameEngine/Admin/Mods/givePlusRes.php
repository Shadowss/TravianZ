...
if ($wood > 0) {
    $database->query(
        "UPDATE " . TB_PREFIX . "users
         SET b1 = IF(b1 < $time, $time + $wood, b1 + $wood)
         WHERE id > 3"
    );
}
...
if ($clay > 0) {
    $database->query(
        "UPDATE " . TB_PREFIX . "users
         SET b2 = IF(b2 < $time, $time + $clay, b2 + $clay)
         WHERE id > 3"
    );
}
...
if ($iron > 0) {
    $database->query(
        "UPDATE " . TB_PREFIX . "users
         SET b3 = IF(b3 < $time, $time + $iron, b3 + $iron)
         WHERE id > 3"
    );
}
...
if ($crop > 0) {
    $database->query(
        "UPDATE " . TB_PREFIX . "users
         SET b4 = IF(b4 < $time, $time + $crop, b4 + $crop)
         WHERE id > 3"
    );
}
...
$logText =
    "Gave res bonuses to all: " .
    "wood=" . (int)($_POST['wood'] ?? 0) . "d, " .
    "clay=" . (int)($_POST['clay'] ?? 0) . "d, " .
    "iron=" . (int)($_POST['iron'] ?? 0) . "d, " .
    "crop=" . (int)($_POST['crop'] ?? 0) . "d";
